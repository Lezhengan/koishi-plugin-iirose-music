...
function apply(ctx, config) {
    ...
    const playPlaylistTimeSleep = [false, ''];
    let nowPlaySong = null;
    const skipListSong = [];
    let sleepPlayList = [];
    let nowMediaSongId = '';
    let playModel = true; // true=顺序 false=随机
    let playModelRe = false; // true=循环
    // ==================== ShadowSee API ====================
    ...
    // ==================== 播放核心 ====================
    async function playNMedia(musicId, nameText) {
        const bot = getBot();
        if (!bot)
            return 'error';
        if (nameText === '[电台') {
            ...
            try {
                ...
                nowMediaSongId = String(mainSong.id);
        ...
        try {
            ...
            nowMediaSongId = String(songInfo.id);
    ...
    async function sendSearchResult(session, keyword, type) {
        dlog('sendSearchResult 开始:', { keyword, type, userId: session.userId });
        const result = await searchList(keyword, 1, type);
        const userId = session.userId ?? '';
        dlog('搜索结果:', { keyword, type, code: result.code, count: result.list.length });
        if (result.code !== 200) {
            waitUser.delete(userId);
        ...
        msg += hintText(type, 1);
        waitUser.set(userId, { list: result.list, offset: 1, keyword, type });
    ...
    // ==================== 队列添加 ====================
    async function addToQueue(session, type, id) {
        try {
            if (type === 'playlist') {
                ...
                for (let i = 1; i <= pages; i++) {
                    ...
                    for (const s of req.songs) {
                        const ar = s.ar || [];
                        playListSong.push([s.id, s.name, ar.map(a => a.name).join('/'), '歌单', listData]);
                    }
                }
            ...
            if (type === 'album') {
                ...
                for (const s of req.songs || []) {
                    const ar = s.ar || [];
                    playListSong.push([s.id, s.name, ar.map(a => a.name).join('/'), '专辑', listData]);
                }
            ...
            if (type === 'radio') {
                ...
                for (const p of req.programs) {
                    playListSong.push([p.id, p.mainSong.name, p.mainSong.artists.map(a => a.name).join('/'), '电台', listName]);
                }
            ...
            if (type === 'artists') {
                ...
                for (const s of req.songs || []) {
                    const ar = s.ar || [];
                    playListSong.push([s.id, s.name, ar.map(a => a.name).join('/'), '歌手', String(id)]);
                }
    ...
    async function handleWaitUser(session, msg) {
        const userId = session.userId ?? '';
        const wait = waitUser.get(userId);
        if (msg === '上一页') {
            if (wait.offset > 1)
                wait.offset--;
        ...
        if (wait.type === 'music') {
            ...
            const status = await playNMedia(item.id, '[单歌');
            if (status === 'error') {
        ...
        if (wait.type === 'radio') {
            ...
            waitUser.set(userId, { list: [], offset: 1, keyword: String(item.id), type: 'radioProgram' });
            await showOffset(session, waitUser.get(userId));
            return true;
        }
        if (wait.type === 'radioProgram') {
            await session.send(atUser(session) + '解析中...');
            const status = await playNMedia(item.id, '[电台');
            if (status === 'error') {
        ...
        await addToQueue(session, wait.type, item.id);
        return true;
    }
    // ==================== 命令处理 ====================
    async function playById(session, text) {
        ...
        const status = await playNMedia(id, '[单歌');
        if (status === 'error') {
            await session.send(atUser(session) + '致命错误，运行崩溃');
        }
    }
    async function addListById(session, type, text) {
        const id = parseIdFromText(text);
        if (id) {
            const code = await addToQueue(session, type, id);
            if (code === 404) {
    ...
    async function showList(session) {
        ...
        for (const item of playListSong) {
    ...
    function modelText() { return playModel ? '顺序' : '随机'; }
    function reText() { return playModelRe ? '是' : '否'; }
    /** 帮助菜单（>帮助 / 帮助 共用） */
    function helpMenu() {
    ...
    async function handleCommand(session, msg) {
        ...
        dlog('handleCommand 进入:', { comHead, rest });
        try {
            if (rest.startsWith('点歌id ')) {
                dlog('执行命令: 点歌id');
                await playById(session, rest.substring(5));
            }
            else if (rest.startsWith('点歌 ')) {
                dlog('执行命令: 点歌');
                await sendSearchResult(session, rest.substring(3), 'music');
            }
                ...
                if (playListSong.length) {
                    playPlaylist = false;
                    if (nowPlaySong && !skipListSong.includes(String(nowPlaySong[0])))
                        skipListSong.push(String(nowPlaySong[0]));
                }
            }
            else if (rest === '跳过') {
                await session.send('cut');
                ...
                const before = playListSong.length;
                playListSong = playListSong.filter(item => item[4] !== name);
                if (playListSong.length !== before)
                    await session.send(`已删除 ${name}！`);
                else
                    await session.send(`未找到包含 ${name} 的媒体`);
            }
            else if (rest.startsWith('歌单 搜索 ')) {
                await sendSearchResult(session, rest.substring(6), 'playlist');
            }
            else if (rest.startsWith('歌单 ')) {
                await addListById(session, 'playlist', rest.substring(3));
            }
            else if (rest.startsWith('专辑 搜索 ')) {
                await sendSearchResult(session, rest.substring(6), 'album');
            }
            else if (rest.startsWith('专辑 ')) {
                await addListById(session, 'album', rest.substring(3));
            }
            else if (rest.startsWith('电台 搜索 ')) {
                await sendSearchResult(session, rest.substring(6), 'radio');
            }
            else if (rest.startsWith('电台 ')) {
                await addListById(session, 'radio', rest.substring(3));
            }
            else if (rest.startsWith('歌手 搜索 ')) {
                await sendSearchResult(session, rest.substring(6), 'artists');
            }
            else if (rest.startsWith('歌手 ')) {
                await addListById(session, 'artists', rest.substring(3));
            }
            else if (rest === '模式') {
                await session.send(`当前状态：\n播放模式：${modelText()} 循环播放：${reText()}`);
            }
    ...
    ctx.on('message', async (session) => {
        ...
        if (!msg)
            return;
        const userId = session.userId ?? '';
        dlog('收到消息:', { userId, username: session.username, content: msg });
        // 选择交互（命令优先）
        if (waitUser.has(userId) && !msg.startsWith(comHead) && msg !== '<TT1') {
    ...
    ctx.on('iirose/music-play', async (session, data) => {
        try {
            ...
            if (!songId && data.owner === bot.selfId)
                songId = nowMediaSongId;
            dlog('music-play 事件:', { link: data.link, url: data.url, owner: data.owner, selfId: bot.selfId, parsedSongId: songId, nowMediaSongId, musicHot });
            if (!songId)
                return;
            if (String(songId) === playPlaylistTimeSleep[1])
                playPlaylistTimeSleep[0] = true;
            if (!musicHot)
                return;
    ...
    async function playTick() {
        ...
        if (playModelRe) {
            if (playModel) {
                playSong = playListSong.shift();
                playListSong.push(playSong);
            }
            else {
                playSong = playListSong[Math.floor(Math.random() * playListSong.length)];
            }
        }
        else {
            if (playModel) {
                ...
                const idx = Math.floor(Math.random() * playListSong.length);
                playSong = playListSong.splice(idx, 1)[0];
            }
        }
        nowPlaySong = playSong;
        playPlaylistTimeSleep[1] = String(playSong[0]);
        const songTime = await playNMedia(playSong[0], `[${playSong[3]}`);
        if (songTime === 'error' || songTime === 0) {
            playPlaylist = false;
            return;
        }
        sleepPlayList = [songTime, String(playSong[0])];
    }
    async function sleepTick() {
        if (!sleepPlayList.length)
            return;
        if (!playPlaylistTimeSleep[0])
            return;
        playPlaylistTimeSleep[0] = false;
        const [duration, songId] = sleepPlayList;
        await (0, koishi_1.sleep)(duration * 1000);
        if (!skipListSong.includes(songId)) {
            playPlaylist = false;
        }
        else {
            const idx = skipListSong.indexOf(songId);
            if (idx > -1)
                skipListSong.splice(idx, 1);
        }
        sleepPlayList = [];
    }
    ...
    ctx.command('点歌 [keyword:text]', `网易云音乐点歌，${comHead}帮助 查看完整菜单`)
        ...
        await sendSearchResult(session, keyword.trim(), 'music');
    });
    ctx.command('点歌.id <id:text>', '按id播放歌曲')
        ...
        await playById(session, id.trim());
    });
    ...
    ctx.command('点歌.歌单 [操作:text]', '播放歌单')
        ...
        if (text.startsWith('搜索 ')) {
            await sendSearchResult(session, text.substring(3).trim(), 'playlist');
        }
        else {
            await addListById(session, 'playlist', text);
        }
    });
    ctx.command('点歌.专辑 [操作:text]', '播放专辑')
        ...
        const text = operation.trim();
        if (text.startsWith('搜索 ')) {
            await sendSearchResult(session, text.substring(3).trim(), 'album');
        }
        else {
            await addListById(session, 'album', text);
        }
    });
    ctx.command('点歌.电台 [操作:text]', '播放电台')
        ...
        const text = operation.trim();
        if (text.startsWith('搜索 ')) {
            await sendSearchResult(session, text.substring(3).trim(), 'radio');
        }
        else {
            await addListById(session, 'radio', text);
        }
    });
    ctx.command('点歌.歌手 [操作:text]', '播放歌手作品')
        ...
        const text = operation.trim();
        if (text.startsWith('搜索 ')) {
            await sendSearchResult(session, text.substring(3).trim(), 'artists');
        }
        else {
            await addListById(session, 'artists', text);
        }
    });
